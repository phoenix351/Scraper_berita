...
class KompasScraper(scrapy.Spider):
    ...
    sampai = "2020-03-07"
    dari = "2019-01-01"
    rentang = date_range(dari, sampai).tolist()
    for i in range(0, len(rentang)):
        rentang[i] = rentang[i].strftime("%Y-%m-%d")
    print(rentang)

    url = 'https://indeks.kompas.com/?site=all&date='
    url_list = [(url + i) for i in rentang]
    print(url_list)
    ...
    def parse_artikel(self, response):
        ...
        tag_selector = 'li.tag__article__item a ::text'
        ...
        tag_list = response.css(tag_selector).get_all()
